...
def apply_to_club(request, club_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            if not(Application.objects.filter(club=desired_club, user = current_user).exists()) and not(Membership.objects.filter(club=desired_club, user = current_user).exists()):
...
def show_applications_to_club(request, club_id):
    ...
    if not(Membership.objects.filter(club=club_to_view, user=request.user, is_owner=True).exists()):
        # Access denied
...
def respond_to_application(request, app_id, is_accepted):
    ...
    if not(Membership.objects.filter(club=application.club, user=request.user, is_owner=True).exists()):
        # Access denied
        return redirect('show_clubs')
...
def promote_member_to_officer(request, member_id):
    ...
    member = Membership.objects.get(id = member_id)
    club = member.club
    if not(Membership.objects.filter(club=club, user=request.user, is_owner=True).exists()):
